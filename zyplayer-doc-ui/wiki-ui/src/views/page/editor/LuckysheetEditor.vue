<template>
  <div class="luckysheet-editor-container">
    <div id="luckysheet-page" class="luckysheet-content" style="margin:0px;padding:0px;position:absolute;width:100%;height:100%;left: 0px;top: 0px;"></div>
  </div>
</template>
...
// 直接以 ESM 方式引入 luckysheet 及样式，避免动态脚本在不同环境下导出形式不一致导致初始化失败
import $ from 'jquery';
...
const initLuckysheet = async () => {
  ...
  const luckysheetLib = mod && (mod.default || mod);
  if (!luckysheetLib) {
