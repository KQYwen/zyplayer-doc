<template>
  <div style="min-height: 100%" class="space-manage-vue">
    ...
    >
      <div style="text-align: right; margin-bottom: 10px">
        <span style="float: left; line-height: 40px">
          仅展示我收藏的空间：
          <el-switch
            v-model="userSetting.wiki_only_show_favorite"
            inactive-value="0"
            active-value="1"
            @change="wikiOnlyShowFavoriteChange"
          ></el-switch>
          <el-tooltip
            class="item"
            effect="dark"
            content="控制左上角空间下拉列表仅展示我收藏的空间"
            placement="top-start"
          >
            <el-icon
              style="vertical-align: middle; margin-left: 10px; color: #999"
        ...
        <el-button
          ...
          >刷新</el-button
        >
        <el-button type="primary" @click="showCreateSpace" :icon="ElIconPlus"
          >创建空间</el-button
        >
      </div>
      <el-table
      ...
      >
        ...
        <el-table-column label="开放地址">
          <template v-slot="scope">
            <el-button
              type="text"
              @click="showOpenSpace(scope.row.uuid)"
              v-if="scope.row.openDoc == 1"
        ...
        <el-table-column prop="favorite" label="收藏" width="60">
          <template v-slot="scope">
            <el-icon class="favorite-icon"><el-icon-star-on /></el-icon>
            <el-icon class="favorite-icon"><el-icon-star-off /></el-icon>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220">
          <template
            v-if="userSelfInfo.id == scope.row.createUserId"
            v-slot="scope"
          >
            <el-button
              size="small"
              type="primary"
              v-on:click="editSpaceInfo(scope.row)"
              >编辑</el-button
            >
            <el-button
              size="small"
              type="warning"
              v-on:click="editSpaceAuth(scope.row)"
              >授权</el-button
            >
            <el-button
              size="small"
              type="danger"
              v-on:click="deleteSpaceInfo(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-dialog title="权限管理" v-model="spaceAuthDialogVisible" width="900px">
      ...
      <el-table
        :data="spaceAuthGroupList"
        border
        style="width: 100%; margin: 10px 0"
      >
        <el-table-column prop="groupId" label="分组名" width="150">
          <template v-slot="scope">{{
            searchGroupMap[scope.row.groupId]
          }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="80">
          <template v-slot="scope">
            <el-button
              ...
              type="danger"
              plain
              @click="deleteGroupSpaceAuth(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
      ...
      <div style="text-align: right">
        ...
        <el-button type="primary" v-on:click="saveGroupSpaceAuth"
          >保存配置</el-button
        >
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    addSpaceAuthUserGroup() {
      ...
      if (
        !!this.spaceAuthGroupList.find(
          (item) => item.groupId == this.spaceAuthNewGroupId
        )
      ) {
    ...
    saveGroupSpaceAuth() {
      let param = {
        spaceId: this.editSpaceId,
        authList: JSON.stringify(this.spaceAuthGroupList),
      }
    ...
    deleteGroupSpaceAuth(row) {
      this.spaceAuthGroupList = this.spaceAuthGroupList.filter(
    ...
    editSpaceAuth(row) {
      ...
      userApi.userGroupList().then((json) => {
        ...
        this.searchGroupList.forEach(
          (item) => (this.searchGroupMap[item.id] = item.name)
        )
      })
    ...
    wikiOnlyShowFavoriteChange() {
      let param = {
        ...
        value: this.userSetting.wiki_only_show_favorite,
      }
