        ...
        <el-tab-pane :label="newsTypesMap[searchParam.newsType]" name="first">
          ...
          <div v-else class="line-box" v-for="item in spacePageNews">
            ...
            <div class="page-preview-box">
              <div
                class="page-preview-title"
                v-on:click="showPageDetail(item)"
                v-html="item.pageTitle"
              ></div>
              <div
                class="page-preview-content"
                v-html="item.previewContent"
              ></div>
              <div>
                <span
                  ><img src="../../assets/img/zan.png" class="zan-img" />
                  {{ item.zanNum }}　</span
                >
                <span
                  ><el-icon class="view-img"><el-icon-view /></el-icon>
                  {{ item.viewNum }}　</span
                >
...
export default {
  ...
  methods: {
    ...
    initQueryParam(to) {
      ...
      this.newsTypesMap = {}
      this.newsTypesArr.forEach(
        (item) => (this.newsTypesMap[item.key] = item.val)
      )
    },
  },
}
